...
//float ecValuef=0.084;
//float humiValuef=50.74;
int result = 0; //1-success 2-failure
int chedo= 0; //0-allow 1-unallow
int kieutruyen=0;
int dem=0;
char* checkwr;
...
char hihi[length+1];
  ...
  if(checkwr!=NULL){
    ...
    else if(function == "3" && address_receive == "B"){
...
void loop() {
  ...
  float a[10], b[10], humi, temp;
	...
	if (millis() - timepoint > 50) //time interval: 1s
	{ 
		 timepoint = millis();
     digitalWrite(De,0);
     digitalWrite(Re,0);
...
digitalWrite(De,1);
    digitalWrite(Re,1);
    if(kieutruyen==0){
  guidata(ecValue,humi,"B","A");}
  else guidata(ecValue,humi,"B","D");
  delay(100);
  chedo=1;}
